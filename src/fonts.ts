// Static imports of all @fontsource font CSS files
// This ensures Vite bundles the woff2 font files into the build output

import '@fontsource/ma-shan-zheng/index.css';
import '@fontsource/zhi-mang-xing/index.css';
import '@fontsource/long-cang/index.css';
import '@fontsource/liu-jian-mao-cao/index.css';
import '@fontsource/noto-sans-sc/index.css';
import '@fontsource/noto-sans-tc/index.css';
import '@fontsource/noto-sans-hk/index.css';
import '@fontsource/noto-serif-sc/index.css';
import '@fontsource/noto-serif-tc/index.css';
import '@fontsource/noto-serif-hk/index.css';
import '@fontsource/wdxl-lubrifont-sc/index.css';
import '@fontsource/wdxl-lubrifont-tc/index.css';
import '@fontsource/zcool-kuaile/index.css';
import '@fontsource/zcool-qingke-huangyou/index.css';
import '@fontsource/zcool-xiaowei/index.css';
import '@fontsource/chiron-hei-hk/index.css';
import '@fontsource/chiron-goround-tc/index.css';
import '@fontsource/huninn/index.css';
import '@fontsource/bpmf-huninn/index.css';
import '@fontsource/bpmf-iansui/index.css';
import '@fontsource/bpmf-zihi-kai-std/index.css';
import '@fontsource/iansui/index.css';
import '@fontsource/lxgw-wenkai-tc/index.css';
import '@fontsource/lxgw-wenkai-mono-tc/index.css';
import '@fontsource/lxgw-marker-gothic/index.css';
import '@fontsource/cactus-classical-serif/index.css';
import '@fontsource/chocolate-classical-sans/index.css';
import '@fontsource/uoqmunthenkhung/index.css';
import '@fontsource/dela-gothic-one/index.css';
import '@fontsource/black-han-sans/index.css';
import '@fontsource/zen-maru-gothic/index.css';
import '@fontsource/m-plus-rounded-1c/index.css';
import '@fontsource/kosugi-maru/index.css';
import '@fontsource/zen-kaku-gothic-new/index.css';
import '@fontsource/m-plus-1p/index.css';
import '@fontsource/sawarabi-gothic/index.css';
import '@fontsource/sawarabi-mincho/index.css';
import '@fontsource/hina-mincho/index.css';
import '@fontsource/shippori-mincho/index.css';
import '@fontsource/biz-udmincho/index.css';
import '@fontsource/biz-udpgothic/index.css';
import '@fontsource/kiwi-maru/index.css';
import '@fontsource/klee-one/index.css';
import '@fontsource/cherry-bomb-one/index.css';
import '@fontsource/darumadrop-one/index.css';
import '@fontsource/yusei-magic/index.css';
import '@fontsource/yuji-mai/index.css';
import '@fontsource/chokokutai/index.css';
import '@fontsource/aoboshi-one/index.css';
import '@fontsource/noto-serif-jp/index.css';
import '@fontsource/noto-sans-jp/index.css';

// No exports needed - side effects only
